...
public record Day4(string Input) : AdventDay(Input)
{
    ...
    public static int CountWordOccurrencesInGrid(string word, string[] grid)
    {
        int count = 0;
                ...
                for (int dy = -1; dy <= 1; dy++)
                {
                    for (int dx = -1; dx <= 1; dx++)
                    {
                        ...
                        for (int i = 0; i < word.Length; i++)
                        {
                            ...
                            if (offsetY < 0 || offsetY >= grid.Length || offsetX < 0 || offsetX >= grid[offsetY].Length || grid[offsetY][offsetX] != word[i])
                            {
                                foundWord = false;
                                break;
                            }
    ...
    private static bool IsMasX(string[] grid, (int, int) center)
    {
        ...
        var deltas = new (int, int)[] { (-1, -1), (1, -1) };
        ...
        foreach (var delta in deltas)
        {
            ...
            if (!IsOnGrid(start, grid) || !IsOnGrid(end, grid) || grid[start.Item2][start.Item1] != MirroredChar(grid[end.Item2][end.Item1]))
            {
                return false;
            }
        }
    ...
    {
        return position.Item2 >= 0 && position.Item2 < grid.Length && position.Item1 >= 0 && position.Item1 < grid[position.Item2].Length;
    }
